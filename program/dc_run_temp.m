function dc_run_temp(policy_select, filename, temperature)

red_temperature = temperature; %default 18
...
for i=[0:17:250,250]
    if i==0 || i==250
        ...
        switch  policy_select
            ...
            case 'dvf_cpu'
                task_schedule=zeros(1,25);
                index = 25;
                for j=1:i
                    machine_id = priority_array(index);
                    task_schedule(machine_id)=task_schedule(machine_id)+1;
                    index= index -1;
                    if index == 0
                        index = 25;
                    end
                end
                [P_TOTAL,P_AC,P_CMP,T_sup,COP,job_dis,res_cvx] = dc_schedule(i, 470, task_schedule, coeff_file, red_temperature)
        end
    end
