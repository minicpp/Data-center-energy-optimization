...
cvx_solver
opt_total_result = zeros(3,18);
opt_ac_result = zeros(3,18);
opt_cmp_result = zeros(3,18);
for jobs=1:18
    %% Task Predefined
    ...
    rest_calc = size(shadow,2)+1;
        ...
        switch method
            case 0
                opt_total_result(1,jobs) = P_AC;
                opt_total_result(2,jobs) = P_CMP;
                opt_total_result(3,jobs) = P_TOTAL;
            case 1
                opt_ac_result(1, jobs) = P_AC;
                opt_ac_result(2, jobs) = P_CMP;
                opt_ac_result(3, jobs) = P_TOTAL;
            case 2
                opt_cmp_result(1, jobs) = P_AC;
                opt_cmp_result(2, jobs) = P_CMP;
                opt_cmp_result(3, jobs) = P_TOTAL;
        end
        ...
        opt_ac_result
        opt_cmp_result
        dlmwrite('inlet_total.txt',opt_total_result);
        dlmwrite('inlet_ac.txt',opt_ac_result);
        dlmwrite('inlet_cmp.txt',opt_cmp_result);
