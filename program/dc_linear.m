%cvx_solver SeDuMi (suggested)
function [P_TOTAL,P_AC,P_CMP,T_sup,COP,Job_distribute,Res_cvx] = dc_linear(job_size, job_calc_req, Apath, red_temperature)
cvx_solver Mosek
...
while task_n >= 0
    ...
    maximize (T_sup)
    %% power getting
    P_e == P_e_min + (P_e_max - P_e_min)/max_freq * f
    if(task_n > 0)
        f == y*task + freq_allocated
        f <= max_freq
    else
        f == freq_allocated
    end
...
fprintf('\nPower Allocation:\n');
p_e = P_e_min + (P_e_max - P_e_min)/max_freq * calc_req * job_allocated
